...
class Parser extends Admin_my_controller
{
    ...
    public function pcsParser($iEtappe)
    {
        ...
        //$data = read_file('temp2.txt');

        //var_dump($data);

        /*        $cCharAt = stripos($data, '<div class="page-content');
                var_dump($cCharAt);
                $data = substr($data, $cCharAt);
                $cCharEnd = stripos($data, '<div class="w30 right');
                var_dump($cCharEnd);
        ...
        $etappentyp = $this->model->getEtappenTyp($iEtappe);

        if ($etappentyp['etappen_klassifizierung'] == 3 || $etappentyp['etappen_klassifizierung'] == 5) {
            $rowsTime = ['rang', 'gc-pos', 'gc-rueck', 'startnummer', 'h2h', 'namen', 'age', 'team', 'ptsuci', 'ptspcs', 'bonifications', 'rueckstand', 'avg'];
        } else {
            $rowsTime = ['rang', 'gc-pos', 'gc-rueck', 'startnummer', 'h2h', 'namen', 'age', 'team', 'ptsuci', 'ptspcs', 'bonifications', 'rueckstand'];
        }
        ...
        */
        ...
        if ($etappen_nr['etappen_nr'] == 1) {
            $rowsMtn = ['rang', 'startnummer', 'h2h', 'namen', 'age', 'team', 'uci', 'pnts', 'berg'];
            $rowsPts = ['rang', 'startnummer', 'h2h', 'namen', 'age', 'team', 'uci', 'pnts', 'points'];
            $rowsGk = ['rang', 'startnummer', 'h2h', 'namen', 'age', 'team', 'ptsuci', 'rueckstand'];
        } else {
            $rowsGk = ['rang', 'prv', 'updown', 'startnummer', 'h2h', 'namen', 'age', 'team', 'ptsuci', 'bonifications', 'rueckstand'];
            $rowsPts = ['rang', 'prv', 'updown', 'startnummer', 'h2h', 'namen', 'age', 'team', 'uci', 'pnts', 'points'];
            //$rowsMtn = ['rang', 'startnummer', 'namen', 'age', 'team', 'berg'];
            $rowsMtn = ['rang', 'prv', 'updown', 'startnummer', 'h2h', 'namen', 'age', 'team', 'uci', 'pnts', 'berg'];
/*            $rowsMtn = ['rang', 'startnummer', 'h2h', 'namen', 'age', 'team', 'uci', 'pnts', 'berg'];
            $rowsPts = ['rang', 'startnummer', 'h2h', 'namen', 'age', 'team', 'uci', 'pnts', 'points'];*/
            //$rowsGk = ['rang', 'startnummer', 'h2h', 'namen', 'age', 'team', 'ptsuci', 'rueckstand'];
        }
        ...
        /*		$rowsGk = ['rang', 'startnummer', 'age', 'namen', 'team', 'ptsuci', 'gc-pos', 'gc-time', 'rueckstand', 'timewonlost'];
        ...
        foreach ($html->find('.result-cont') as $resultCont) {
            $basicCount = 0;
                    ...
                    if ($basicCount === 0) {
                        ...
                        foreach ($element->find('tr') as $row) {
                            foreach ($row->find('td') as $td) {
                                ...
                                //var_dump($td->text());
                                if ($c == 0) {
                                    //echo $rowsTime[$i] . ': ' . $td->text() . '-';
                                    if ($rowsTime[$i] === 'rueckstand') {
                                        //var_dump($td->text());
                                    }
...
/*        echo '<pre>';
        ...
        foreach ($aTables as $kC => $vC) {
            foreach ($vC as $k => $v) {
                ...
                if ($kC == 'tag') {
                    if ($v['rang'] == 1) {
                        $rueckstand = 0;
                    } else if (strip_tags($v['rang']) != 'DNF' && strip_tags($v['rang']) != 'DNS' && strip_tags($v['rang']) != 'OTL') {
                        $rueckstand = strip_tags(trim(substr($v['rueckstand'], strpos($v['rueckstand'], ' '))));
                        $tmp = explode(':', $rueckstand);
                        $newRueckstand = $tmp[0] . ':' . substr($tmp[1], 0, 2);
                        $rueckstand = $newRueckstand;
                        $temp = chr(44) . chr(44) . ' ';
                        if ($rueckstand == $temp) {
                            $rueckstand = $rueckstandAlt;
                        } else {
                            $rueckstandAlt = $rueckstand;
                        }
                    }
    ...
    public function parseTest()
    {
        ...
        $this->db->where('etappe', 215);
        $dataToday = $this->db->get()->result_array();

        $this->db->select('fahrer_id, rueckstand');
        $this->db->from('temp_gk');
        $this->db->where('etappe', 214);
        ...
        $i = 1;
        foreach ($dataToday as $k => $v) {
            $data = array('etappen_id' => 215, 'fahrer_id' => $v['fahrer_id'], 'rueckstand' => $v['resultat'], 'rueckstandOhneBS' => $v['resultat'], 'rang' => $i);
