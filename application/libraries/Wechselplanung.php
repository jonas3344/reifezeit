...
class Wechselplanung {
	...
	public function __construct($iEtappe, $iPlanungId = 0) {
		...
		$this->CI->db->where('pk.etappen_id', $this->aEtappe['etappen_id']);
		$this->CI->db->where('p.id', $iPlanungId);
		$this->CI->db->join('planung p', 'p.id=pk.planung_id');
		$this->aAlleKaderAktuell = $this->CI->db->get('planung_kader pk')->result_array();
		$this->aAlleKaderAktuell = $this->_arrayNewIndex($this->aAlleKaderAktuell);
		
		if ($this->aPastStage != null) {
			$this->CI->db->where('pk.etappen_id', $this->aPastStage['etappen_id']);
			$this->CI->db->where('p.id', $iPlanungId);
			$this->CI->db->join('planung p', 'p.id=pk.planung_id');
			$this->aAlleKaderGestern = $this->CI->db->get('planung_kader pk')->result_array();
